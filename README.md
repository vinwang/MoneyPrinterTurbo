...
WebUI 选择“本地文件”作为视频来源后，可以在“设置 → 素材来源设置”中分别填写一个
本地视频素材库目录和一个背景音乐库目录。程序会递归读取子目录，并将子目录名作为初始
分类；首次使用或素材发生变化时，会抽取视频代表帧并调用当前配置的 OpenAI-compatible
视觉模型建立本地 SQLite 索引，之后只处理新增或变化的文件；素材目录发生变化后可在设置
面板点击“更新本地素材索引”。

```text
/Users/you/video-library/
├── 人物/
├── 产品/
└── 城市/

/Users/you/music-library/
├── 轻快/
├── 科技/
└── 温暖/
```

填写目录后，输入或生成视频文案即可自动显示分镜候选；每个分镜提供一个首选和两个备选。
选择“智能匹配本地音乐”时会根据整篇文案显示本地 BGM 候选。原有 TTS、上传人声和其他
背景音乐模式不受影响。上传的本地素材默认只用于当前任务，勾选“同时将上传素材加入素材库”
后才会复制到配置的视频素材库。

索引文件保存在 `storage/asset_library.sqlite3`。视觉分析失败会显示具体失败状态，不会
静默改用未经分析的素材；如果素材库目录不可读或文件被移除，也会在索引和匹配界面明确提示。
